...
class ImageTooBigException(Exception):
    """[Exception if the size of the image is too large]"""
...
def format_exception(exc):
    class_name = exc.error_type if hasattr(exc, "error_type") and exc.error_type else exc.__class__.__name__
    detail = {
        "errors": [],
    ...
    if isinstance(exc.detail, dict):
        for error_key, error_values in list(exc.detail.items()):
            if isinstance(error_values, dict):
                # Special case for ListSerializer validation errors
                for __, inside_error_value in error_values.items():
                    detail["errors"].append(
                        {
                            "message": inside_error_value,
                        },
                    )
            else:
        ...
        for error_msg in exc.detail:
            detail["errors"].append({"message": error_msg} if not isinstance(error_msg, dict) else error_msg)
    else:
        detail["errors"].append(
            {
